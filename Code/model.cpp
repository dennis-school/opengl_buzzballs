...
/**
 * @brief Model::unitize makes sure the model fits inside a unit sized box
 *
 ...
 */
void Model::unitize() {
    // Note that this method makes sure that it fits exactly in a unit-sized box
    //   but only with its current orientation. (e.g. a rotated unit-sized box
    //   does not fit inside a unit-sized box, because it's "corners stick out")
    ...
    float size = std::max( std::max( maxX - minX, maxY - minY ), maxZ - minZ );
    ...
    float offZ = ( maxZ + minZ ) / 2;

    for ( QVector3D& v : vertices ) {
        v -= QVector3D( offX, offY, offZ );
        v /= size;
    }
    for ( QVector3D& v : vertices_indexed ) {
        v -= QVector3D( offX, offY, offZ );
        v /= size;
    }
}
